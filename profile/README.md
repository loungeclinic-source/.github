<div align="center">

<img src="https://img.shields.io/badge/Lounge%20Clinic-Organization-4A90D9?style=for-the-badge&logo=github&logoColor=white" />
<img src="https://img.shields.io/badge/Flutter-Mobile-02569B?style=for-the-badge&logo=flutter&logoColor=white" />
<img src="https://img.shields.io/badge/Status-Active-brightgreen?style=for-the-badge" />

# 🏥 Lounge Clinic

> A modern healthcare platform connecting patients with clinic services —  
> seamlessly across mobile, web, and backend.

</div>

---

## 📦 Repositories

| Repo | Stack | Description |
|------|-------|-------------|
| `mobile-frontend` | Flutter | 📱 Patient-facing mobile app for iOS & Android |
| `dashboard-frontend` | TypeScript | 🖥️ Internal admin dashboard for clinic management |
| `backend` | PHP | ⚙️ Core API — data, logic, and integrations |

---

## 🏗️ Architecture Overview

```
┌─────────────────┐     ┌───────────────────┐
│   Mobile App    │     │  Admin Dashboard  │
│   (Flutter)     │     │    (Web)          │
└────────┬────────┘     └────────┬──────────┘
         │                       │
         └──────────┬────────────┘
                    │
           ┌────────▼─────────┐
           │    Backend API   │
           │   (Hostinger)    │
           └──────────────────┘
        
```

---

## 🌐 Our Platforms

| Platform | URL |
|----------|-----|
| 🏥 Main Website | [loungeclinic.net](https://loungeclinic.net) |
| 💚 Health Platform Company | [wealthlifehealth.net](https://wealthlifehealth.net) |

---

## 🛠️ Tech Stack

**Mobile**
![Flutter](https://img.shields.io/badge/Flutter-02569B?style=flat&logo=flutter&logoColor=white)
![Dart](https://img.shields.io/badge/Dart-0175C2?style=flat&logo=dart&logoColor=white)


**Dashboard**
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)

**Backend**
![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat&logo=php&logoColor=white)


**Infrastructure**
![Hostinger](https://img.shields.io/badge/Hostinger-673DE6?style=flat&logo=hostinger&logoColor=white)
![GoDaddy](https://img.shields.io/badge/GoDaddy-1BDBDB?style=flat&logo=godaddy&logoColor=black)

---

## 📬 Contact

For business or technical inquiries:

📧 [admin@loungeclinic.net](mailto:admin@loungeclinic.net)

---

<div align="center">
<sub>© Lounge Clinic · All rights reserved</sub>
</div>
